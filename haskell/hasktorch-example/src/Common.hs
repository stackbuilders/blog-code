...
import Torch.Typed
import Torch.Typed.Vision (MnistData (..))
...
train initModel initOptim forward' learningRate ptFile dataPath = do
  ...
  where
    --trainStep :: LearningRate device dtype -> (model -> Bool -> t -> IO (Tensor device dtype '[batchSize, seqLen]))
    --  -> (model, optim)
    --  -> ListT IO (t, Tensor device 'Int64 '[batchSize])
    --  -> IO (model, optim)
    trainStep lr forward'' init' lst=(P.foldM step' begin done . enumerateData) lst
      where
        step' (model, optim) ((input, target), _) = do
          
          --print $ "what is a: " <> show a
